...
class GatedSelfAttention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, mask=False, weight=None):
        ...
        self.setzero = torch.ones(config.num_medterms) - torch.eye(config.num_medterms)
        self.setzero = self.setzero.to(torch.device('cuda'))
...
#         self.s1 = nn.Linear(nx // config.n_head, config.num_medterms)
#         self.s2 = nn.Linear(config.num_medterms, config.num_medterms)
#         self.act_fn1 = nn.LeakyReLU()

#         self.h1 = nn.Linear(nx // config.n_head, nx // config.n_head)
#         self.h2 = nn.Linear(nx // config.n_head, nx // config.n_head)
...
#         vals1 = self.s1(x)
#         vals = self.act_fn1(vals1)
#         vals2 = self.s2(vals)
        
#         alpha = mySoftmax(vals2)
#         valh1 = self.h1(x)
#         valh2 = self.h2(x)
#         for i in range(alpha.size(0)):
#             for j in range(alpha.size(1)):
#                 for k in range(alpha.size(2)):
#                     alpha[i][j][k][k] = 0
#         alpha = torch.matmul(alpha, valh1)
#         h = valh2 + alpha
#         h = self.act_fn2(h)
...
# TODO Multi-Head Attention: Image and tag
class GatedEncoderAttention(nn.Module):
    def __init__(self, nx, config, scale=False):
        super(GatedEncoderAttention, self).__init__()
        n_state = nx  # in Attention: n_state=768 (nx=n_embd)
        # [switch nx => n_state from Block to Attention to keep identical to TF implem]
        assert n_state % config.n_head == 0
...
class EncoderBlock(nn.Module):
    def __init__(self, n_ctx, config, scale=False):
        ...
        nx = config.n_embd
        self.attn = GatedEncoderAttention(nx, config, scale)
        self.ln_1 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config)
        self.ln_2 = LayerNorm(nx, eps=config.layer_norm_epsilon)

        self.attn2 = GatedSelfAttention(nx, n_ctx, config, scale, mask=False)
...
class EncoderModel(OpenAIGPTPreTrainedModel):
    def __init__(self, config):
        super(EncoderModel, self).__init__(config)
...
class LMHeadModel(OpenAIGPTPreTrainedModel):
    def __init__(self, config):
    ...
    def forward(self, att_feats, input_ids=None, position_ids=None, token_type_ids=None, mode='train'):
        if mode == 'train':
            return self._forward(att_feats, input_ids, position_ids, token_type_ids)
        else:
            return self._sample(att_feats)

    def _forward(self, att_feats, input_ids, position_ids=None, token_type_ids=None):
