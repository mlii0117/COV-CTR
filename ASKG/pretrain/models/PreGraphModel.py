...
WEIGHTS_NAME = "en_model.bin"

WEIGHTS_NAME = "ch_model.bin"
...
class OpenAIGPTConfig(object):
    ...
    def __init__(
        self,
        vocab_size_or_config_json_file='./ch_log/transformer-config.json',
        n_special=0,
        n_positions=512,
        n_ctx=512,
        n_embd=768,
...
class SelfAttention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, mask=False):
...
class LMHeadModel(nn.Module):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None):
        hidden_states = self.decoder(input_ids, position_ids, token_type_ids)
